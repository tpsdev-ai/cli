...
Then `launchctl load` both. The office-connect daemon logs to `~/.tps/logs/office-tps-reed.log` and exposes connection state at `~/.tps/connections/reed.json` (last heartbeat, reconnect count, messages sent/received).
...
**Cause:** Pre-`cli@0.6` the outbox-drain loop could race with concurrent writers, hit a partial JSON file, and throw. The unhandled exception killed the daemon (see [tps-cli#281](https://github.com/tpsdev-ai/cli/pull/281)). Fixed by atomic writes + defense-in-depth try/catch.
...
- **Branch identity** is a long-lived Ed25519 keypair generated by `tps branch init` and stored under `~/.tps/identity/` on the branch. Persisted across daemon restarts.
- **Host identity** is similarly long-lived on the host and stored under `~/.tps/identity/` on the host.
